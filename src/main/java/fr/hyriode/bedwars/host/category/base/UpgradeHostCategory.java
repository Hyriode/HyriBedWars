    ...
    public static class UpgradeGUI extends BWHostCategory {
        ...
        public UpgradeGUI(Upgrade upgrade) {
            ...
            for (Upgrade.Tier tier : upgrade.getTiers()) {
                this.addOption(slot(i++, 3), new PreciseIntegerOption(BWHostUtils.optionDisplay("upgrade-" + upgrade.getName() + "-price-tier-" + tier.getTier(), tier.getIcon()), tier.getPrice().getAmount().apply(null).getDefaultValue(), 0, 1000, new int[] { 1, 10 }));
            }
        }
    }
}
