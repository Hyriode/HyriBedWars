...
public class ShopCategoryHostCategory extends BWHostCategory {
    public ShopCategoryHostCategory(ShopCategory value) {
        ...
        List<MaterialShop> materialsShop = HyriBedWars.getShopManager().getItemShopByCategory(false, value);
        int i = 0;
        for(int y = 0; y < size.getHeight(); ++y){
            for(int x = 0; x < size.getWidth(); ++x){
                if(materialsShop.size() > i) {
                    ...
                    this.addSubCategory(slot(size.getStartX() + x, size.getStartY() + y), new ShopItemHostCategory(materialShop));
                } else {
                    break;
                }
            }
